...
As linhas `egress host=código` são o **probe de rede**: `200`/`301`/`302` = domínio liberado no allowlist do ambiente; `000` (o proxy recusa o CONNECT) ou `403` = bloqueado. É o único teste de egress da execução — não repetir depois.

**Leitura direta é `curl` pelo Bash, nunca `WebFetch`** (mesmo com o domínio liberado no allowlist, o `WebFetch` usa outro caminho de rede e continua bloqueado — só o `curl`/Bash é governado pelo allowlist do ambiente). Domínio liberado → `curl` + extração por script; bloqueado → WebSearch, como até aqui.

Todo "hoje" — assunto, cabeçalho, janela, dia do ano — vem daí, nunca de e-mails ou notícias. Data futura ou muito antiga em qualquer item é armadilha, não novidade. O total do Munger é o **contado agora**, nunca um número lembrado; reproduza a `obs` quando houver. A saída do Bash nesta nuvem é UTF-8 — se algum caractere sair estranho, leia a citação pela ferramenta de arquivo antes de publicar.
...
- **Web** (`sonnet`): Rota 13 (Congresso — Últimas Leis Publicadas) e MPVs; concursos; ConJur e Migalhas. **Cole no prompt, a partir do ledger e do probe do passo 1:** (i) o resultado do probe por domínio e a regra de leitura — **liberado: `curl` pelo Bash** (`WebFetch` é outro caminho de rede e continua bloqueado, não tente) nas listagens `https://conjur.com.br/noticias/`, `/artigos/` e `/colunas/` (apex **sem** `www`, que só redireciona 301; `/ultimas-da-conjur/` foi extinta, dá 404), `https://www.migalhas.com.br/quentes` e `https://www.congressonacional.leg.br/materias/ultimas-leis-publicadas` (este **exige** `www.`) — extrair títulos/links/datas por script (`python3 -c` com regex). **Data confiável:** ConJur e Congresso trazem no slug/página (ConJur: `<h2 class="conjur-posts-tipo-titulo"[^>]*><a href="https://conjur.com.br/AAAA-mes-DD/slug/">` — o `[^>]*` é obrigatório, a tag vem com `style=`); **Migalhas não se data por `curl`** (SPA sem timestamp no HTML): confirmar cada item por WebSearch e descartar o que não datar. **Bloqueado: direto ao WebSearch.** Mais as armadilhas `congresso_mpv` e `banca_edital_antigo`, uma linha cada; (ii) a lista `pendencias_vivas` de concursos com `id`, `texto` e `faixa`; (iii) a regra de faixa: **1ª execução da semana** (segunda, ou 1ª após gap) → reverificar `ativa` e `longo_prazo`; **demais dias** → reverificar só `ativa` e buscar apenas edital/inscrição/banca/resultado **novos** (≤ 6 buscas de concursos). Sem isso o subagente re-varre tudo do zero todo dia.

Cada notificação de coletor termina com uma linha de uso (`subagent_tokens`, `tool_uses`, `duration_ms`): anote os três números — vão em `coletores` no passo 8, e são a única evidência para promover, rebaixar ou reabsorver um coletor. Relatório vazio **não** é ausência de novidade: quem interpreta ausência é você. Coletor incompleto ou incoerente, refaça a fonte você mesmo — mas **erro determinístico (403/404/DNS/EGRESS_BLOCKED) não se repete na mão**: fallback documentado e registrar a falha. Coletor interrompido pelo harness: `SendMessage` para o mesmo agente pedindo para concluir — ele retoma do transcript; reiniciar do zero só se não houver progresso registrado. Enquanto os coletores rodam, faça o versículo (passo 6) — não sonde nem agende wakeup; a notificação de término chega sozinha.
...
**Versículo (você, não subagente) — 1 comando, sem WebSearch:**

```
python3 work/versiculo.py --data AAAA-MM-DD --dia N
```

(`--data` e `--dia` = a data e o `doy` do passo 1.) O script tenta o YouVersion e, em seguida, o DailyVerses pela página datada, **confere que a data impressa na página é a pedida** e devolve JSON com `fonte`, `referencia`, `versao`, `texto` e `url` — publique exatamente isso, rotulando a fonte real que ele reporta (nunca atribua ao YouVersion o que veio do fallback). Ele já implementa a decisão do usuário de 07/09/2026: primeira fonte que responder com versículo datado de hoje vence, sem cruzar nem desempatar.

Saiu `"ok": false` (exit 1)? O JSON lista o que falhou em cada fonte. Aí, e só aí, vale o fallback manual: até 2 buscas WebSearch (`bible.com verse-of-the-day day=N`, depois `dailyverses.net/AAAA/M/D`), publicando a primeira com referência e texto claros para hoje. Nada disso resolvendo, a seção fica pendente — **nunca inventar versículo**.
...
3. As 13 entradas do índice têm seção correspondente, na ordem do template — inclusive o **bloco OPINIÃO/ANÁLISE dentro da seção Mídia** e a seção Munger.
4. Nenhum `{{` sobrando; nenhum wrapper (`t.rdsv2.net`, `google.com/url`); **selos só da lista fechada** do prompt mestre §2 (nunca inventar — ex.: "SEM NOVIDADE" não existe; usar `EM ANDAMENTO` ou `PENDÊNCIA`).
