...
> **Versão 9 — 07/09/2026.** Este documento contém apenas o que **muda uma decisão em tempo de execução**: identidade visual, seções, protocolos de fonte e regras editoriais. O passo a passo operacional está na tarefa agendada (local ou nuvem, conforme onde a rotina roda); o estado corrente das fontes, no ledger. O histórico de versões e a narrativa dos incidentes vivem no `resumo-legislativo-historico.json` — não aqui, porque eram relidos todo dia sem alterar nenhuma decisão. **Nesta versão:** Seção 13 ganhou um fallback para o versículo do dia — decisão do usuário em 07/09/2026: se o YouVersion não puder ser confirmado, usar a primeira fonte paralela que responder com um versículo datado de hoje, sem cruzar nem desempatar entre fontes.
...
- **Fonte primária: YouVersion (bible.com).** Ler `https://www.bible.com/pt/verse-of-the-day` (ou, se WebFetch/curl direto estiver bloqueado no ambiente — ver ledger `fontes[youversion]` e armadilha `cloud_egress_bloqueado` —, buscar por ele via WebSearch). Ao ler a página diretamente, **validar a data com pergunta neutra** ("qual data aparece na página?") — nunca sugerir a data esperada, para não induzir a confirmação. O parâmetro `?day=N` (dia do ano) pode ajudar a checar se um resultado de busca é de hoje ou de um dia anterior.
- **Fallback (decisão do usuário em 07/09/2026):** se o YouVersion não puder ser confirmado com segurança (egress bloqueado e WebSearch não devolve algo datado e claro para hoje, ou qualquer outra falha de acesso), **usar a primeira fonte paralela que responder com um versículo datado de hoje** — ex.: DailyVerses.net, Bíblia Online, BibleGateway, SBB, Bíblia.com — **sem ordem de preferência fixa**: a primeira que der uma resposta utilizável vale. **Ignorar divergência entre fontes** — não gastar esforço tentando várias fontes paralelas para "desempatar" ou escolher a mais correta; apps/sites diferentes legitimamente mostram versículos diferentes no mesmo dia, e isso nunca foi (nem é agora) erro ou pendência. Rotular no e-mail a fonte realmente usada — nunca atribuir a "YouVersion" um versículo que veio do fallback.
- **Nunca inventar versículo.** Só quando NENHUMA fonte (YouVersion nem nenhuma paralela) devolver algo datado e legível para o dia corrente é que a seção fica pendente. Para traduções protegidas longas: referência, versão, link e trecho curto; depois, reflexão própria de 2 a 3 linhas.
