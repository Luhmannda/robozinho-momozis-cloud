...
- **Envio:** feito pelo Apps Script `work/EnviarRobozinho.gs`, que localiza o rascunho pelo **assunto exato**. Envios já ocorreram bem fora da janela nominal (14h15 em 10/07; 20h23 em 06/07) — **nunca presumir que rascunho atrasado ficará sem envio**: criar sempre, a qualquer hora.
...
- **Ambiente (Windows, PowerShell 5.1) — evita round-trip perdido:** não há `node` nem `python`; contas e contagens saem de PowerShell ou `grep`. Comando PowerShell vai na ferramenta **PowerShell**, nunca via Bash (o Bash come o `$_`). **A saída do console mente na acentuação nos dois sentidos:** todo texto acentuado que vá ao e-mail — citação do Munger, relatório do helper — lê-se com `Read`/`grep`, jamais pelo console. Coletor em background avisa sozinho ao terminar: não agendar wakeup nem ficar sondando.
...
- **Gravação: sempre pelo helper.** Escrever a execução num `.json` separado e chamar `work/Update-Historico.ps1` (parâmetros `-NovaExecucaoJson` — **caminho do arquivo, não o JSON inline**; o script faz `Test-Path` e falha silenciosamente com exit code 2 se receber conteúdo direto —, `-ReconciliarId`, `-ReconciliarMessageId`, `-RelatorioPath`). Ele faz merge, reconciliação, arquivamento, detecção de mojibake e emite o estado. **Nunca escrever um `.ps1` novo com o conteúdo embutido:** acentos digitados dentro de um script são lidos com codepage errada e corrompem os campos novos. Validar lendo o arquivo do `-RelatorioPath` — **a saída do console não serve para validar acentuação**, pois a codepage do console mente nos dois sentidos.
...
- Fonte principal: ler `https://www.bible.com/pt/verse-of-the-day` e **validar a data local com pergunta neutra** (o conteúdo exibido deve corresponder ao dia da execução; nunca sugerir a data esperada no prompt da leitura — ver Seção 0).
- Se a YouVersion falhar ou a data não bater, fontes alternativas em português, nesta ordem: DailyVerses (pt), Bíblia Online, Sociedade Bíblica do Brasil, BibleGateway (pt). *(Não usar o parâmetro `?day=N`.)*
- **Regra de desempate:** se fontes legítimas divergirem entre si (ocorreu em 10/07/2026: YouVersion = Mateus 18:20; DailyVerses = Marcos 12:33), prevalece a **YouVersion**, desde que a data validada corresponda ao dia da execução; registrar a divergência em `verification_notes`.
- **Nunca inventar versículo.** Para traduções protegidas longas: referência, versão, link e trecho curto; depois, reflexão própria de 2 a 3 linhas.

## 14. Seção "Conhecimento nunca é demais"

- Usar uma citação curta de Charlie Munger de `C:\Users\perei\Claude Code\work\munger-quotes.json` (banco curado no repositório).
